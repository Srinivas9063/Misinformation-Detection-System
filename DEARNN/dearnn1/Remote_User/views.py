...
def Predict_Tweet_Meesage_Type(request):
    if request.method == "POST":
        ...
        vector1 = cv.transform(tweettext1).toarray()
        ...
        if prediction == 0:
                ...
                val= 'Misinfomration gender'
        elif prediction == 2:
                val = 'Misinfomration religion'
        elif prediction == 3:
                val = 'Misinformation other_cyberbullying'
        elif prediction == 4:
                val = 'Misinformation age'
        elif prediction == 5:
                val = 'Misinformation ethnicity'

        print(prediction)
        print(val)

        Cyberbullying_Detection_Type.objects.create(Tweet_Message=tweettext,Prediction=val)
